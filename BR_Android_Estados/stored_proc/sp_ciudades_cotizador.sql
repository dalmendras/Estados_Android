...
DROP PROCEDURE IF EXISTS `sp_ciudades_cotizador` $$
CREATE DEFINER=`root`@`%` PROCEDURE `sp_ciudades_cotizador`()
BEGIN

SELECT *
FROM
	(SELECT DISTINCT 1 AS punto, t3.regcodigo, 	t0.ciudcodigo, t0.ciudnombre
	...
	FROM
		...
		FROM
			(
			SELECT DISTINCT 2 AS punto, t3.regcodigo, t3.regorden,
			...
			UNION ALL
			SELECT DISTINCT 2 AS punto, t9.regcodigo, t9.regorden,
